...
try {
    if (external_fs_.existsSync(MLPROJECT_PATH)) {
        console.log('MLproject file found');
    }
    else {
        throw new Error('MLproject file missing!');
    }
    const mlproject = load(external_fs_.readFileSync(MLPROJECT_PATH, 'utf8'));
    ...
    try {
        const modelcard = load(external_fs_.readFileSync(modelcard_path, 'utf8'));
